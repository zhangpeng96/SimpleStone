...
{
    //图片设置
    $favicon = new Typecho_Widget_Helper_Form_Element_Text('favicon', NULL, NULL, _t('Favicon'), _t('输入 favicon 的链接（含 http/https），若不填则会使用主题的 Logo'));
    $form->addInput($favicon);
    $logo = new Typecho_Widget_Helper_Form_Element_Text('logo', NULL, NULL, _t('Logo'), _t('输入 Logo 的链接（含 http/https），留空则使用主题自带 Logo'));
    $form->addInput($logo);

    //Pjax加速
    $pjax = new Typecho_Widget_Helper_Form_Element_Checkbox('pjax',
    array(
      'InstantClick' => _t('启用 Instantclick 预加载'),
      ),
    array('InstantClick') ,
    _t('Instantclick 预加载'),
    _t('启用 Instantclick 预加载以达到网站加速目的。为了兼容性，请到「设置-评论」中关闭「反垃圾保护」'));
...
function tagsExisted($input) {
    if (count($input->tags) >= 1) {
        return true;
    } else if (count($input->tags) == 0) {
        return false;
    } else {
        return false;
    }
...
function commentsExisted($input) {
    if ($input->commentsNum >= 1) {
        return true;
    } else if ($input->commentsNum == 0) {
        return false;
    } else {
        return false;
    }
